...
| 내면 아이 찾기 워크샵(₩99,000) | `workshop_intake_purchases` | `workshop_intake` |

> ℹ️ 워크샵(`workshop_intake_purchases`)만 tid 를 `payment_key` 가 아닌 **`tid`** 컬럼에,
> 결제일시를 **`confirmed_at`** 에 둔다. 환불 API·어드민 화면은 셀렉트 별칭(`payment_key:tid`)으로
> 이 차이를 자동 흡수하므로 호출부는 다른 상품과 동일하게 쓰면 된다.
...
await fetch("/api/payment/nicepay/cancel", {
  ...
  body: JSON.stringify({
    type: "counseling",          // husband_match | counseling | workshop | minds_relationship | workshop_intake
