 ...
 * 동작(SDK 대기 / 결제 수단 선택 / 로그인 라우팅)은 컨텍스트에서 일괄 처리.
 * 클릭하면 결제 수단 선택 모달이 열리고, 거기서 고른 수단으로 NicePay 결제창이 뜬다.
 */
export function WorkbookBuyButton({ className, children }: Props) {
  const { openModal, isSubmitting, sdkPending } = useWorkshopCheckoutCtx();
