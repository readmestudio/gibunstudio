...
import { CHILDHOOD_SENTENCES } from "@/lib/missions/childhood-sentence";

type Props = { onSubmit?: (data: Record<string, string>) => void; submitted?: boolean };

export function ChildhoodSentenceMission({ onSubmit, submitted = false }: Props) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-[var(--foreground)]/80">
        아래 문장의 뒷부분을 맨 먼저 떠오르는 생각으로 완성해 주세요. 솔직한 마음을 그대로 나타내 주세요.
      </p>
      ...
      <div className="space-y-4">
        {CHILDHOOD_SENTENCES.map((sentence) => (
