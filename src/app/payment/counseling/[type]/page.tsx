...
const COUNSELING_CONFIG: Record<string, { title: string; price: number }> = {
  "inner-child": { title: "내면 아이 해석 상담", price: 80000 },
  couple: { title: "커플 내면 아이 상담", price: 150000 },
  package: { title: "패키지 상담", price: 200000 },
  individual: { title: "1:1 개인 상담", price: 100000 },
};
...
export default function PaymentCounselingPage() {
  const params = useParams();
  const type = params.type as string;
  const config = COUNSELING_CONFIG[type] ?? { title: "상담", price: 0 };
  const [copied, setCopied] = useState(false);

  const finalPrice = Math.max(0, config.price - DISCOUNT);
    ...
    <div className="mx-auto max-w-xl px-4 py-16">
      ...
      <div className="mt-8 rounded-xl border-2 border-[var(--accent)] bg-[var(--accent-muted)] p-6">
        ...
        <p className="mt-4 text-sm text-[var(--foreground)]/70">
          입금 확인 후 코치가 승인하면 상담 예약이 가능합니다.
        </p>
      </div>

      <div className="mt-8 flex gap-4">
        <Link
          href="/payment/counseling/complete"
