 ...
 *
 * 브라우저에서만 일어나는 깔때기 이벤트(②③)를 받아 (1) funnel_events 에 기록하고
 * (2) 운영자 슬랙으로 중계한다. 슬랙 봇 토큰은 서버 전용이라, 클라이언트가 직접 슬랙에
 * 못 쏜다 — 이 라우트가 다리.
 ...
 *    단 뮤트는 슬랙에만 적용되고, funnel_events 기록은 항상 남는다(notified=false).
 *
 * Body: {
...
export async function POST(req: NextRequest) {
  ...
  // 이벤트 화이트리스트 밖은 400.
  const EVENTS = ["test_start", "reached_paywall", "checkout_click"] as const;
  ...
  // variant 유실로 minds 로 폴백된 값)는 슬랙 발송만 건너뛴다(클라이언트 dedupe 유지).
  //
  ...
  // 다시 /minds 광고를 전면 켜면 아래 muted 조건만 지우면 된다.
  const muted = variant !== "inner_child" && event !== "checkout_click";

  // 기록은 뮤트와 무관하게 항상 남긴다 — 뮤트는 '슬랙 노이즈 차단'이지 '이벤트가
  // 일어나지 않았다'는 뜻이 아니다. 종전엔 여기서 바로 return 해버려 뮤트된 이벤트가
  // 어디에도 남지 않았고, 그래서 /minds 퍼널은 집계 자체가 불가능했다.
  after(() => recordFunnelEvent({ funnel: variant, event, leadId, notified: !muted }));
