...
const CARD_TITLES: Record<number, { title: string; subtitle?: string; card_type: ReportCard['card_type'] }> = {
  ...
  3: { title: '', subtitle: '', card_type: 'personality' }, // title, subtitle 모두 LLM이 생성
...
### 카드 3: 당신은 ___ 타입 (2500-3000자) ← 가장 긴 카드!
⚠️ 응답 형식 주의: "3_subtitle", "3_title", "3"을 따로 작성하세요!
⚠️ MBTI/애니어그램 번호 명시 금지! 특징만 서술!

- "3_subtitle": 영어 타입명 (예: "Emotional Explorer Type", "Quiet Growth Seeker Type")
- "3_title": "당신은 {한국어 타입이름} 타입이에요" 형식 (예: "당신은 감성적 탐구자 타입이에요")
- "3": 본문 내용 (2500-3000자)

본문 작성 규칙:
1. 맥락이 바뀌는 문단마다 **키워드: 요약 한 줄**을 볼드로 먼저 작성
   예) **당신의 강점: 차갑지만 부드러운 리더십**
   예) **관계 패턴: 깊이보다 넓이를 택하는 사람**
   예) **숨겨진 욕구: 인정받고 싶은 마음**
2. 볼드 요약 시작 전 빈 줄 2개 삽입 (문단 여백)
3. 각 섹션 500-600자 이상으로 상세히 작성

예시 형식:
"""

**당신의 핵심 특성: 감각을 통해 세상을 이해하는 사람**

여기에 첫 번째 문단 내용... (500자 이상)


**관계에서 보이는 패턴: 넓지만 선별적인 연결**

여기에 두 번째 문단 내용... (500자 이상)


**숨겨진 욕구: 깊이 이해받고 싶은 마음**

여기에 세 번째 문단 내용... (500자 이상)
"""
...
반드시 아래 JSON 형식으로만 응답:
{
  ...
  "3_subtitle": "영어 타입명 (예: Emotional Explorer Type)",
  "3_title": "당신은 {한국어 타입이름} 타입이에요",
  "3": "카드3 본문 (2500-3000자, 볼드 키워드 요약 + 빈 줄 포함)",
  "4": "카드4 내용 (700자 이상)",
  "5": "카드5 내용 (700자 이상)",
  "6": "카드6 내용 (700자 이상)",
  "7": "카드7 내용 (700자 이상)",
...
다음 카드에서는 이 데이터를 바탕으로 당신의 성격 유형을 더 깊이 분석해드릴게요.`,
    ...
    // 영어 타입명 생성
    "3_subtitle": `${sortedCats[0]?.id === 'healingSpirituality' ? 'Introspective Seeker' :
      sortedCats[0]?.id === 'careerBusiness' ? 'Ambitious Achiever' :
      sortedCats[0]?.id === 'entertainmentVlog' ? 'Social Connector' :
      sortedCats[0]?.id === 'musicMood' ? 'Sensory Explorer' :
      sortedCats[0]?.id === 'readingHumanities' ? 'Intellectual Curator' :
      sortedCats[0]?.id === 'sportsOutdoor' ? 'Active Adventurer' :
      'Balanced Explorer'} Type`,

    // 한국어 타입명
    "3_title": `당신은 ${sortedCats[0]?.id === 'healingSpirituality' ? '내면 탐구형' :
      sortedCats[0]?.id === 'careerBusiness' ? '야망 있는 성취형' :
      sortedCats[0]?.id === 'entertainmentVlog' ? '사교적 연결형' :
      sortedCats[0]?.id === 'musicMood' ? '감각 탐험형' :
      sortedCats[0]?.id === 'readingHumanities' ? '지적 큐레이터형' :
      sortedCats[0]?.id === 'sportsOutdoor' ? '활동적 모험형' :
      '균형 잡힌 탐험형'} 타입이에요`,

    "3": `#${topTCI[0]?.name || '성장지향'} #${topTCI[1]?.name || '깊은대화'} #자기이해


**당신의 핵심 특성: ${sortedCats[0]?.id === 'healingSpirituality' ? '내면의 깊이를 탐구하는 사람' :
  sortedCats[0]?.id === 'careerBusiness' ? '목표를 향해 달려가는 사람' :
  sortedCats[0]?.id === 'entertainmentVlog' ? '사람들과 연결되는 것을 즐기는 사람' :
  sortedCats[0]?.id === 'musicMood' ? '감각을 통해 세상을 이해하는 사람' :
  '다양한 관심사를 탐험하는 사람'}**

당신의 구독 리스트를 분석한 결과, 매우 흥미로운 패턴을 발견했어요. ${sortedCats[0]?.name || '다양한'} 카테고리가 전체의 ${sortedCats[0]?.percent || 0}%를 차지하고 있어요. 이 숫자가 의미하는 바가 있습니다.

이런 채널을 많이 구독하시는 분들은 ${sortedCats[0]?.id === 'healingSpirituality' ? '삶의 의미와 내면의 평화를 중요하게 여기는' : sortedCats[0]?.id === 'careerBusiness' ? '성장과 성취를 추구하는' : sortedCats[0]?.id === 'entertainmentVlog' ? '즐거움과 소통을 즐기는' : sortedCats[0]?.id === 'musicMood' ? '분위기와 감각적 경험을 중시하는' : '다양한 경험을 추구하는'} 경향이 있어요. 단순히 "이런 영상을 좋아한다"는 것을 넘어서, 당신의 깊은 내면에서 이런 콘텐츠를 갈망하고 있다는 뜻이에요.

두 번째로 많은 ${sortedCats[1]?.name || '기타'}(${sortedCats[1]?.percent || 0}%)와의 조합이 특히 인상적이에요. ${sortedCats[0]?.name || ''}의 깊이와 ${sortedCats[1]?.name || ''}의 균형을 동시에 추구한다는 건, 당신이 한 가지에만 매몰되지 않고 삶의 다양한 측면을 고려한다는 의미예요. 이런 균형감각은 흔하지 않아요.

왜 이렇게 해석하냐면, 구독 채널은 무의식적인 관심사를 가장 정직하게 반영하기 때문이에요. 당신이 의식적으로 선택하지 않았더라도, 알고리즘이 추천하는 채널에 반응한다는 건 그 분야에 내재된 욕구가 있다는 의미예요. 특히 새벽에 혼자 보는 영상, 습관적으로 클릭하는 썸네일—거기에 당신의 진짜 관심사가 숨어 있어요.


**관계에서 보이는 패턴: ${data.mbti_scores.E > data.mbti_scores.I ? '넓지만 선별적인 연결' : '깊고 의미 있는 관계 추구'}**

분석 결과, 당신은 ${mbtiTraits[0]} 특성이 있어요. 이는 ${data.mbti_scores.E > data.mbti_scores.I ? '타인과의 교류에서 에너지를 얻고, 활발한 소통을 즐기는' : '혼자만의 시간을 통해 에너지를 충전하고, 깊은 생각을 즐기는'} 분이라는 뜻이에요. 관계에서도 이 특성이 그대로 드러나요.
...
'당신은 깊은 관계를 선호하는 타입이에요. 많은 사람과 피상적으로 어울리기보다, 소수의 사람과 진정한 연결을 맺는 것을 더 가치 있게 여겨요. 처음에는 다가가기 어려워 보일 수 있지만, 일단 마음을 열면 깊고 진실된 관계를 만들어가요.'}

또한 ${mbtiTraits[1]} 경향도 보여요. ${data.mbti_scores.S > data.mbti_scores.N ? '현실적이고 구체적인 정보를 선호하며, 실용적인 것에 가치를 두는' : '추상적인 아이디어와 가능성을 탐구하며, 미래지향적인 사고를 하는'} 특성이죠. 이것이 관계에서는 ${data.mbti_scores.S > data.mbti_scores.N ? '상대방의 실제 행동과 현실적인 측면을 중요하게 보는' : '상대방의 가능성과 성장 잠재력을 보는'} 방식으로 나타나요.


**숨겨진 욕구: ${t.ST >= 50 ? '깊이 이해받고 싶은 마음' : t.RD >= 50 ? '인정받고 싶은 마음' : '자유롭고 싶은 마음'}**

${enneaTrait} 특성이 두드러지는 당신, 이 특성 뒤에는 숨겨진 욕구가 있어요.

${t.ST >= 50 ?
'당신은 "진짜 나"를 알아봐 주는 사람을 갈망해요. 겉으로 보이는 모습이 아니라, 내면의 깊이와 복잡함을 이해해주는 사람. "너는 왜 그렇게 생각해?"라고 진심으로 물어봐 주는 사람. 그런 사람을 만났을 때 비로소 마음을 열 수 있어요.' :
t.RD >= 50 ?
'당신은 자신의 가치를 인정받고 싶은 마음이 있어요. 하지만 이것은 허영이 아니에요. 당신이 노력하는 것, 성취하려는 것, 좋은 사람이 되려고 애쓰는 것—이런 것들을 누군가가 알아봐 주기를 바라는 거예요.' :
'당신은 자유를 갈망해요. 누군가의 기대에 맞추거나, 틀에 갇히는 것을 싫어해요. 하지만 동시에 혼자가 아닌, 함께하면서도 자유로울 수 있는 관계를 원해요. 그런 균형을 찾는 것이 당신에게는 중요한 과제예요.'}

이런 분들은 ${data.enneagram_type === 4 ? '자신만의 독특함을 중요시하고, 깊은 감정을 경험하는 것을 두려워하지 않아요' : data.enneagram_type === 9 ? '평화로운 환경을 만들고, 갈등을 피하며 조화를 추구해요' : data.enneagram_type === 3 ? '목표를 향해 달려가며 성취를 통해 자신의 가치를 확인해요' : '자신의 방식으로 세상과 소통하고 성장해요'}.


**강점과 매력: ${topTCI[0]?.name || '깊이'}에서 빛나는 사람**

이 모든 특성이 조합되어 만들어진 당신은, 정말 독특한 매력을 가진 사람이에요.

당신의 가장 큰 강점은 ${topTCI[0]?.name || '깊이'}에요. ${topTCI[0]?.score || 0}점이라는 높은 점수는 이 영역에서 당신이 특별한 능력을 가지고 있다는 의미예요. ${topTCI[0]?.key === 'ST' ? '삶의 의미를 추구하고 깊이 있는 사고를 하는' : topTCI[0]?.key === 'SD' ? '스스로 결정하고 이끌어가는' : topTCI[0]?.key === 'CO' ? '타인과 연결되고 공동체를 소중히 여기는' : '당신만의 방식으로 세상을 탐험하는'} 능력이 뛰어나요.

다른 사람들은 당신을 처음 만났을 때 ${data.mbti_scores.E > data.mbti_scores.I ? '따뜻하고 친근한' : '차분하고 신중한'} 인상을 받을 거예요. 하지만 시간이 지날수록 당신의 깊이와 진정성을 발견하게 되죠. "알면 알수록 매력 있는 사람"—그게 바로 당신이에요.

특히 ${topTCI[1]?.name || '보완적 강점'}도 ${topTCI[1]?.score || 0}점으로 높아서, ${topTCI[0]?.name || ''}과 함께 시너지를 만들어내요. 이 조합은 흔하지 않아요.


**왜 이렇게 분석했는지: 데이터가 말해주는 것**

이 분석은 단순한 추측이 아니에요. 다음과 같은 데이터가 일관되게 같은 방향을 가리키고 있어서 신뢰할 수 있어요:

• ${sortedCats[0]?.name || ''} ${sortedCats[0]?.percent || 0}% → ${characteristics[0] || '성장 추구'} 성향의 강력한 신호
• ${topTCI[0]?.name || ''} ${topTCI[0]?.score || 0}점 → 이 영역에서 뚜렷한 특성
• ${topTCI[1]?.name || ''} ${topTCI[1]?.score || 0}점 → 보완적 강점으로 시너지 형성
• ${sortedCats[1]?.name || ''} ${sortedCats[1]?.percent || 0}% + ${sortedCats[2]?.name || ''} ${sortedCats[2]?.percent || 0}% → 균형 잡힌 관심사
• 구독 패턴과 심리 특성의 일관성 확인됨

이 모든 데이터가 하나의 그림을 그리고 있어요. 그리고 그 그림은 바로 당신이에요.`,
...
export async function runPhase1FromPrecomputed(
  ...
  for (let cardNumber = 1; cardNumber <= 9; cardNumber++) {
    ...
    // 카드 2, 3의 title은 LLM 응답에서 가져옴
    let title: string;
    if (cardNumber === 2) {
      title = cardContents['2_title'] || '당신의 취향을 분석해보았어요';
    } else if (cardNumber === 3) {
      title = cardContents['3_title'] || '당신은 감성적 탐구자 타입이에요';
    } else {
      title = meta.title;
    }

    // 카드 3의 subtitle도 LLM 응답에서 가져옴
    let subtitle: string | undefined;
    if (cardNumber === 3) {
      subtitle = cardContents['3_subtitle'] || 'Emotional Explorer Type';
    } else {
