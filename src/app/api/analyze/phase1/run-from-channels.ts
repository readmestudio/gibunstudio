...
// 카드 1 고정 텍스트 (브릿지 인트로)
const CARD_1_FIXED_CONTENT = `이 리포트는 당신의 성격과 기질, 관계 패턴, 갈등 방식, 스트레스 반응, 행복의 조건, 그리고 당신에게 맞는 배우자 타입까지 알아봐요.

연애할 때는 몰랐던 것들이, 함께 살기 시작하면 하나씩 드러나요. 결혼을 결심하기 전에, 나를 먼저 정확히 아는 게 중요한 이유예요.

YouTube 구독 목록은 아무도 보지 않을 때의 당신이에요. 꾸미지 않은 관심사가 성격을 가장 솔직하게 보여주는 데이터예요. 여기서부터 시작할게요.`;
...
const CARD_TITLES: Record<number, { title: string; subtitle?: string; card_type: ReportCard['card_type'] }> = {
  ...
  6: { title: '당신을 행복하게 만드는 것', subtitle: '행복의 조건', card_type: 'values' },
  7: { title: '이런 사람 만나면 숨이 막혀요', subtitle: '딜브레이커', card_type: 'values' },
...
## 카드 4: 관계 패턴 + 스트레스 반응 — 같은 상황, 같은 반응

- "4": 본문 (1500-1750자) — 아래 구조를 반드시 포함:
  1) **"평소 관계에서의 당신"** — → 요약 한 줄(비유/상징 표현 사용) + 일상적 관계 행동 패턴 2개 + 각각에 인사이트. RD/CO 조합에서 도출.
  2) **"갈등이 오면"** — → 요약 한 줄(비유 사용) + 1문단. 갈등 해결 방식(HA/SD/CO 조합). 회피형/직면형/타협형 중 패턴. 행동으로 묘사.
  3) **"압력을 받으면"** — → 요약 한 줄(비유 사용) + 1문단. 스트레스 상황에서의 반응 패턴. 감정 억제/폭발, 트리거, 회복 방식을 간결하게. (HA/RD/ST 조합 기반)
  4) **"이렇게 생각할 수도 있어요"** — → 요약 한 줄(비유 사용) + 1문단. 이 반복 패턴과 스트레스 반응이 사실은 자기를 보호하는 전략이라는 인사이트. ("리프레이밍" 단어 사용 금지)
...
## 카드 6: 행복의 조건 — 당신을 행복하게 만드는 것 (1500-1750자)
- "6": 아래 구조를 반드시 포함:
  ...
  3) **"강점으로 읽기"** — → 요약 한 줄(비유 사용) + 1문단. 행복의 조건을 알고 있다는 것 자체가 관계에 진심이라는 인사이트. ("리프레이밍" 단어 사용 금지)

## 카드 7: 딜브레이커 — 이런 사람 만나면 숨이 막혀요 (1500-1750자)
- "7": 아래 구조를 반드시 포함:
...
## ⚠️ 유튜브 채널명 금지. 사주 용어 금지. TCI 점수 금지.
## ⚠️ 카드 3-5에서 이미 다룬 성격/관계/스트레스/행복 특성을 재설명하지 마세요. "파트너"에 대해서만 이야기합니다.
## ⚠️ 행동 패턴을 보여준 뒤, "왜 이런 행동을 하는지" 인사이트 한 줄로 꿰뚫으세요.
## ⚠️ 각 볼드 섹션 타이틀 바로 아래에 "→ 핵심 요약 한 줄"을 넣으세요.
...
- "5": 아래 구조를 반드시 포함:
  ...
  1) **"당신의 배우자는 이런 사람이에요"**
     → 요약 한 줄
     · 비유 문구: "${h.variant === 'extrovert' ? (h.metaphor_e || '') : (h.metaphor_i || '')}" 활용
     · 배우자 유형명(${h.name}), 카테고리(${h.category} > ${h.subcategory}), 성향(${h.variant === 'extrovert' ? '외향형' : '내향형'}) 소개
     · 이 파트너의 핵심 특성을 1~2문단으로 설명

  2) **"왜 이 사람인가"**
     → 요약 한 줄
     · 사용자의 분석 결과를 근거로 매칭 이유를 논리적으로 설명
     · ${topBottom.top1.name}(1위) + ${topBottom.bottom.name}(최하위) 축 활용
     · ⚠️ 주어 혼동 방지: "당신은" / "이 파트너는" 구분
     · 흐름: "당신은 ~한 사람 → 이 파트너가 바로 그런 사람"

  3) **"이 사람이 구독하고 있을 유튜브 채널"**
     → 요약 한 줄
...
${t.RD >= 50
? '힘든 날, 일단 누군가에게 연락해요. "그랬구나"라는 한마디면 돼요. 혼자 버티는 게 더 무거운 사람이에요.'
: '힘든 날, 먼저 혼자가 돼요. 감정을 정리하지 않은 채 꺼내면 통제력을 잃을까봐 두려운 거예요. 그래서 시간이 필요한 거예요.'}
...
${t.ST >= 50 ?
`• "별일 없어"가 반복되면, 마음이 멀어지는 걸 느껴요. 서로의 안쪽을 꺼내는 시간이에요. 그게 안 되면 같은 공간에 있어도 혼자인 거예요.\n• "왜 그렇게까지 파고들어?"라는 반응이 오면, 더 이상 시도하지 않게 돼요.` :
t.HA >= 50 ?
`• 갑자기 계획이 바뀌면, 당신 안에서는 작은 지진이 일어나요. 준비한 만큼 좋은 시간을 만들고 싶은 마음이 큰 거예요.\n• 약속을 가볍게 바꾸는 사람 앞에서 점점 기대를 줄이게 돼요.` :
`• 상대가 매일 감정을 쏟아내면, 슬쩍 뒷걸음질 치게 돼요. 각자의 공간이 있어야 관계가 숨 쉴 수 있다는 걸 아는 거예요.\n• 상대가 힘들 때 온 신경을 쏟지만, 정작 자기가 힘들 때는 말을 못 해요.`}


**이 패턴이 관계에서 나타나는 방식**
→ ${t.ST >= 50 ? '상대가 깊이를 원하지 않으면, 먼저 문을 닫게 된다' : t.HA >= 50 ? '변화 앞에서 웃으면서도 속으로는 무너지는 패턴' : '돌봄을 주고도 돌봄을 못 받는 관계의 불균형'}

${t.ST >= 50 ? '깊은 대화를 원하는데 상대가 거기까지 와주지 않으면, 실망이 쌓여요. 그리고 어느 순간 먼저 문을 닫게 돼요.' : t.HA >= 50 ? '계획이 바뀌어도 겉으로는 "그래, 괜찮아"라고 해요. 하지만 속으로는 허탈함이 밀려와요.' : '상대를 돌보느라 자기 감정은 후순위가 돼요. "나 지쳤어"라고 말했을 때 상대가 놀라는 이유예요.'}`,
