/**
 * Phase 2 심층 리포트 카드 8장 생성 프롬프트 (v2 — SCT 원문 삽입)
 *
...
export const PHASE2_CARD_PROMPTS = {
  /**
   * 카드 1: "두 개의 거울"
   * YouTube가 비추는 나 vs 설문이 비추는 나
   * SCT: q15 "진짜 원하는 관계" 원문 인용
   */
  card_01_two_mirrors: (data: Phase2CardDataExtended): string => {
...
■ 이 카드의 목적
YouTube 구독 데이터가 보여주는 '무의식적 나'와 설문이 보여주는 '의식적 나'를 나란히 놓고, 읽는 사람이 "이거 어떻게 알았지?"라는 첫 소름을 느끼게 하는 것.
...
1) 도입: "두 개의 거울"이라는 비유로 시작하세요.
   - 하나는 YouTube라는 거울 (무심코 구독한 채널들이 비추는 당신)
   - 다른 하나는 설문이라는 거울 (스스로 답한 당신)
   - 두 거울이 비추는 모습이 같을 수도, 미묘하게 다를 수도 있다는 설정

2) 본론: YouTube 기반 겉모습과 설문 기반 의식을 대비하세요.
...
■ 분량: 1,800~2,500자`;
  ...
  /**
   * 카드 2: "말하지 않은 욕구"
   * 숨겨진 욕구 + q14 이상적인 하루 + q15 핵심 욕구 원문 인용
   */
  card_02_hidden_desires: (data: Phase2CardDataExtended): string => {
    const desires = data.deepCrossValidation.hiddenDesires;

    return `[카드 2: "말하지 않은 욕구"]

${getWritingToneDirective('balanced')}

■ 이 카드의 목적
데이터에서 추론한 숨겨진 욕구 + 사용자가 직접 쓴 SCT 응답을 연결해서, "내 글을 누군가 진짜로 읽었구나"라는 감동을 주는 것.
...
2) 본론: 숨겨진 욕구 하나하나를 풀어가세요.
   ...
   - q14/q15에서 관련 키워드가 있으면 연결하세요
     예: "당신이 '편안함'이라는 단어를 선택한 이유가 여기에 있어요."

3) 전환: 이 욕구들이 결혼과 연애에서 어떤 의미인지 연결하세요.
   - 욕구를 인정하는 것이 왜 중요한지
   - 파트너에게 이 욕구를 어떻게 표현할 수 있는지 (가상 대화 1개)
     예: "솔직히 말하면... '나는 혼자 있고 싶을 때도 있지만, 그래도 네가 가까이 있어 줬으면 좋겠어'라고요."

■ 이 카드에서 하지 말 것
- "두 개의 거울" 비유 반복 금지 (카드 1에서 사용)
- "자기 일치도" 수치 반복 금지
...
■ 분량: 2,000~3,000자`;
   ...
   */
  card_03_chain_reaction: (data: Phase2CardDataExtended): string => {
...
4) 인지 왜곡 패턴을 **일상 언어로** 묘사하세요 (전문 용어 직접 사용 ❌).
...
■ 분량: 1,800~2,800자`;
   ...
   */
  card_04_relationship_map: (data: Phase2CardDataExtended): string => {
    const tci = data.phase1.tci_scores;
    const dims = data.deepCrossValidation.dimensions;
...
■ 이 카드의 목적
SCT 관계 패턴 응답(q6/q7/q8)을 핵심 소재로, 이 사람의 "관계 지도" — 반복되는 패턴, 부착 유형, 관계에서의 역할 — 을 그려주는 것. 여기에 인지 왜곡이 관계 패턴에 미치는 영향도 자연스럽게 녹이세요.
...
1) 도입: "관계의 지도"라는 프레이밍.
   ...
   - 그것은 실수가 아니라, 자기만의 "관계 지도"를 따라 움직이고 있는 거예요
...
4) 관계에서 이 사람의 역할 (돌보는 쪽? 받는 쪽? 동등한 동반자?)
...
6) 인지 왜곡이 관계 패턴에 미치는 영향을 자연스럽게 녹이세요.
   - 전문 용어 없이, 행동과 장면으로 묘사
   - "작은 갈등에서도 '끝'을 떠올리는 마음이 관계에서 이런 패턴으로 나타나요"
   - 카드 3의 "도미노" 비유와 연결 가능
...
■ 이 카드에서 하지 말 것
- 카드 3의 "도미노" 비유를 그대로 반복하지 마세요 (연결만 가능)
- "부착 유형", "불안형", "회피형", "인지 왜곡" 등 심리학 용어 직접 사용 금지 (행동으로 묘사)
- 카드 2의 숨겨진 욕구 목록 반복 금지
...
■ 분량: 1,800~2,800자`;
  ...
  /**
   * 카드 6: "성장의 문턱"
   * 가장 큰 불일치 + 성장 방향 + 실천 행동
   * SCT: q12 핵심 두려움 + q13 바꾸고 싶은 것
   */
  card_06_growth_threshold: (data: Phase2CardDataExtended): string => {
    const dims = data.deepCrossValidation.dimensions;
    const tci = data.phase1.tci_scores;
...
■ 이 카드의 목적
가장 큰 불일치 차원과 TCI 최하위 축, 그리고 핵심 신념 카테고리를 결합하여, 이 사람이 넘어야 할 "문턱"을 비유적으로 보여주고, 자동사고→핵심신념 체인과 함께 구체적인 실천 행동 3개를 제안하는 것.
...
[★ SCT — 성장/두려움 핵심 소재 2개]
• q12 "결혼이나 깊은 관계를 생각할 때 가장 두려운 것은..." → ${sct(data.survey.q12_deepest_fear)}
• q13 "관계에서 내가 바꾸고 싶지만 잘 안 되는 것은..." → ${sct(data.survey.q13_want_to_change)}
...
1) 도입: "문턱"이라는 비유.
   ...
   - 보편적 행동 패턴: "매번 여기서 멈추는 패턴", "이 지점에서 관계가 달라지는 패턴"

2) ★ q12 핵심 두려움을 "문턱"의 실체로 해석하세요.
   - "당신이 쓴 ${sct(data.survey.q12_deepest_fear?.slice(0, 30))}... 이 두려움이야말로 당신의 문턱이에요."
   - 이 두려움이 관계에서 어떻게 나타나는지 구체적 장면

3) ★ 핵심 신념 카테고리를 자연스럽게 녹이세요.
   - 전문 용어 없이, "가장 깊은 곳에서 작동하는 마음"으로 묘사
   - 사고 체인을 서사적으로 풀어주세요: "하나의 상황에서 이런 생각이 떠오르고 → 이런 규칙이 작동하고 → 가장 깊은 곳에서는 이런 마음이..."
   - "이 마음을 발견한 것만으로도 변화의 문이 열려요"

4) ★ q13 바꾸고 싶은 것을 성장 방향과 연결하세요.
   - "당신이 바꾸고 싶다고 말한 ${sct(data.survey.q13_want_to_change?.slice(0, 30))}... 이것을 알고 있다는 것 자체가 이미 반은 넘은 거예요."

5) 가장 큰 불일치 차원을 "문턱"의 구조적 원인으로 해석하세요.
   - ${biggestGap.dimension}에서 YouTube와 설문의 차이 = 아직 통합하지 못한 양면성

6) TCI 최하위 축 ${tciMin[0]}(${tciMin[1]}점)과 연결하세요.
   - 이 축이 낮다는 것 = 그만큼 성장 가능성이 큰 영역

7) 에니어그램 성장 방향을 자연스럽게 녹이세요.
   - "${growthDir}"

8) **실천 행동 3개** (구체적이고 오늘부터 할 수 있는 것):
   - 관계 속 실천 1개
   - 혼자 할 수 있는 실천 1개
   - 마음 습관 변화 1개

■ 이 카드에서 하지 말 것
- 카드 1-5의 비유/키워드 반복 금지
- "당신은 이미 충분히 잘하고 있어요" 같은 맥락 없는 위로 금지
- 에니어그램 번호 직접 언급 금지
- "인지 왜곡", "핵심 신념", "자동적 사고" 등 전문 용어 사용 금지

■ 분량: 2,000~3,000자`;
  },

  /**
   * 카드 7: "당신에게 맞는 사람, 다시 한번"
   * Phase 1 매칭을 설문으로 교차검증한 심층판
   * SCT: q14 이상적인 하루 원문 인용
   */
  card_07_deep_match: (data: Phase2CardDataExtended): string => {
...
■ 분량: 2,000~2,800자`;
  ...
  /**
   * 카드 8: "당신의 마음 사용 설명서"
   * CBT 실천 도구 3가지 (Mind Over Mood 기반)
   */
  card_08_mind_manual: (data: Phase2CardDataExtended): string => {
    const cbt = data.deepCrossValidation.cbtAnalysis;
    const q10 = data.survey.q10_body_signal;
    const q16 = data.survey.q16_hot_scenario;
    const q17 = data.survey.q17_relationship_rules;

    return `[카드 8: "당신의 마음 사용 설명서"]

${getWritingToneDirective('practical')}

■ 이 카드의 목적
앞 7장의 분석 인사이트를 바탕으로, 이 사람이 일상에서 바로 써먹을 수 있는 "자기 이해 도구" 3가지를 소개하는 것. 치료가 아닌, 자기 자신과 더 잘 지내기 위한 간단한 도구예요.
...
[관계 규칙 (q17)]
① 규칙: ${q17 ? sct(q17.rules) : '(미응답)'}
② 긍정 가정: ${q17 ? sct(q17.positive_assumption) : '(미응답)'}
③ 부정 가정: ${q17 ? sct(q17.negative_assumption) : '(미응답)'}

${cbt ? `[인지 왜곡 패턴]\n${cbt.cognitiveDistortions.map(d => `• ${d.label}`).join(', ') || '(없음)'}` : ''}

■ 작성 지침 — 실천 도구 3가지

## ⚠️ 각 도구 제목(**도구 N: "이름"**) 바로 아래에 "→ 핵심 요약 한 줄"을 넣으세요.
## ⚠️ 틀릴 수 있는 구체적 상황(시간대, 장소, 직업)을 특정하지 마세요.

**도구 1: "마음 일기장 3줄"** (사고 기록지 간소화)
   - 매일 밤 3줄만 쓰면 돼요:
     ① 오늘 마음이 흔들린 상황 1가지
     ② 그 순간 떠오른 생각 (예: ${q16 ? `"${q16.slice(0, 30)}..."처럼` : '"혹시 내 탓인가?"처럼'})
     ③ 다시 생각해보면, 다른 가능성은?
   - 이 사람의 인지 패턴에 맞는 "대안적 사고" 예시를 1개 구체적으로 제시하세요
   - "이건 심리 치료가 아니에요. 자기 자신과 대화하는 방법이에요."

**도구 2: "몸이 보내는 신호"** (신체 반응 알아차리기)
   - ${q10 ? `"${q10}" — 당신이 직접 알려준 이 신호가 올 때` : '불안할 때 몸에서 느껴지는 변화가 올 때'}, 3초만 멈춰보세요.
   - 3초 멈추기 → 심호흡 1번 → "지금 내 몸이 뭘 말하고 있지?" 스스로에게 질문
   - "몸은 마음보다 먼저 알아차려요. 몸의 신호를 읽을 수 있으면, 감정에 휩쓸리기 전에 선택할 수 있어요."
   - 구체적 장면: 연인과의 대화에서 이 신호가 올 때의 실전 적용

**도구 3: "규칙 바꿔 말하기"** (중간 신념 유연화)
   - ${q17?.rules ? `"${q17.rules}" → 이 규칙을` : '"~해야 한다"는 규칙을'} 부드럽게 바꿔 말하는 연습
   - "~해야 한다" → "~하면 좋겠지만, 아니라고 해서 관계가 끝나는 건 아니야"
   - 이 사람의 q17 규칙을 실제로 변환한 예시 1개 제시
   - "규칙 자체가 나쁜 게 아니에요. 다만, 너무 엄격한 규칙은 나도 상대도 숨 막히게 해요."

■ 전체 톤
- "이건 숙제가 아니에요. 해보고 싶을 때, 생각날 때만 하면 돼요."
- 모든 도구를 "자기 이해"와 "관계 개선"이라는 맥락에서 소개
- 각 도구마다 관계 장면 예시 1개씩 (총 3개)

■ 이 카드에서 하지 말 것
- "인지행동치료", "CBT", "사고 기록지" 등 전문 용어 사용 금지
...
■ 분량: 2,000~3,000자`;
   ...
   */
  card_09_letter: (data: Phase2CardDataExtended): string => {
    ...
    return `[카드 9: "당신에게 보내는 편지"]

${getWritingToneDirective('literary')}

■ 이 카드의 목적
9장의 분석을 마무리하는 편지. 이 사람의 데이터에서만 나올 수 있는 구체적인 메시지로, "누군가 나를 이렇게까지 읽어준 적이 있었나"라는 감동을 남기는 것. Hot Thought와 핵심 신념을 자연스럽게 언급하세요.
...
■ 이 카드에서 하지 말 것
- 9장 카드의 내용을 요약 정리하지 마세요 (편지는 요약이 아님)
