/**
 * /minds 유료 리포트 — "내 마음 배역표 + 관계도" 분석 엔진 (₩9,900).
 *
...
export interface RoleSlotAnalysis {
  ...
  protects: string;
  /**
   * "아하 모먼트" — 이 배역을 *왜 반복하는지*의 인과(원인/기원) + 반전 재해석(2~3문장).
   * howItWorks(작동 방식)·protects(보호 논리)를 되풀이하지 말고, 유저가 "아, 내가 이래서
   * 그랬구나!" 하고 깨닫도록 한 겹 더 들어간 통찰. dormant 면 빈 문자열 가능. */
  insight: string;
  ...
  "roles": [
    {
      ...
      "insight": "...",            // ★"아하 모먼트" — 이 배역을 *왜 반복하는지*의 인과 + 반전 재해석 2~3문장. howItWorks·protects 를 되풀이하지 말 것(아래 규칙 6-4 참조). dormant 면 ""
...
6-4. **insight 는 각 배역의 "아하 모먼트"다 — 값을 치른 리포트에서 가장 크게 남을 한 방.** howItWorks(작동 방식)·protects(보호 논리)를 다른 말로 되풀이하면 실패다. 유저가 "아, 내가 *이래서* 그랬구나!" 하고 무릎을 치도록, 반드시 **두 박자**로 쓴다: (박자1 — 원인/기원) 이 배역이 *어쩌다 이렇게 굳었는지*의 인과를, 지금의 반응을 *한때 그를 버티게 해준 학습된 논리/보호 장치*로 연결해 짚는다("언젠가 〈…하면 …해진다〉고 익힌 마음이 아직 …"). 없는 유년 기억·사건 창작 금지 — 답변에서 읽히는 마음의 논리를 가설로만. (박자2 — 반전) 그래서 이 마음이 유저가 자책하던 것과 *다르게* 보인다는 재해석("그러니 이건 〈게으름/예민함/못남〉이 아니라, 사실 〈…를 지키려는 안간힘〉이었던 셈이에요"). dominant/active 배역만 2~3문장으로 밀도 있게 쓰고, dormant 는 "". 배역마다 결이 다르게(리더의 아하 ≠ 추방자의 아하).
