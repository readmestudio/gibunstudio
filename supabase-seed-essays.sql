-- essays 테이블 초기 시드
--
-- CMS 전환 전 하드코딩으로 관리되던 3개 에세이를 DB 로 복구합니다.
-- slug 는 공개 URL 의 일부이므로 절대 바꾸지 말 것. (예: /essays/joy-sorrow-beautiful-mind)
-- ON CONFLICT (slug) DO NOTHING 으로 여러 번 실행해도 안전합니다 — 이미 존재하면 건드리지 않음.
--
-- 실행 후에는 /admin/essays 에서 본문/제목/발행일을 자유롭게 수정할 수 있습니다.

INSERT INTO essays (slug, title, preview, published_at, illustration, body)
VALUES
  (
    'joy-sorrow-beautiful-mind',
    '기쁨, 슬픔, 아름다운 마음',
    '기쁨 다음에 찾아오는 슬픔까지가 아름다운 마음이다.',
    '2026-04-24',
    NULL,
...
나에게 기쁨을 선물해준 부모님께 감사한 마음으로, 나는 이 슬픔을 오래 품어보려 한다. 그렇게 품고 또 품다 보면, 언젠가 이 모든 시간은 하나의 아름다운 돌처럼 남아 있을 것이다.$body$
  ),
  (
    'compare-success',
    '남들의 성공에 조급해지는 마음에게',
